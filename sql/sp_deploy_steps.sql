...
BEGIN
    ...
    CALL BD_ADMIN_INFRA.SH_DEPLOY.SP_CREATE_DATABASE(:V_COUCHE, :V_ENV) INTO :V_RET;
    RETURN V_RET;
EXCEPTION
    WHEN OTHER THEN
...
DECLARE
    ...
    V_RA_EX          VARCHAR;
    V_RA_OPS         VARCHAR;
    V_CREATE_RES     VARCHAR;
    V_SCHEMA_APP_SOURCE VARCHAR;
    V_SCHEMA_OFFRE      VARCHAR;
    V_SCHEMA_USE_CASE   VARCHAR;
    V_LOG_SQL        VARCHAR;
...
BEGIN
    ...
    V_SCHEMA_APP_SOURCE := IFF(V_COUCHE = 'BRZ', NULLIF(V_APP_SOURCE, ''), NULL);
    V_SCHEMA_OFFRE      := IFF(V_COUCHE = 'GLD', NULLIF(V_OFFRE, ''), NULL);
    V_SCHEMA_USE_CASE   := IFF(V_COUCHE = 'GLD', NULLIF(V_USE_CASE, ''), NULL);

    CALL BD_ADMIN_INFRA.SH_DEPLOY.SP_CREATE_SCHEMA(
        :V_COUCHE,
        :V_ENV,
        :V_STREAM,
        :V_ENV_PROJET,
    ...
    IF (STARTSWITH(TRIM(V_CREATE_RES), 'ERROR')) THEN
        RETURN V_CREATE_RES;
    END IF;
