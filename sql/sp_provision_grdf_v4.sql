-- ============================================================================
-- PROCÉDURE : BD_ADMIN_INFRA.SH_DEPLOY.SP_PROVISION_GRDF_V4
-- ============================================================================
-- DESCRIPTION :
--   Provisionnement complet d'un stream GRDF sur les 3 environnements GRDF
--   (CONCEPTION, HOMOLOGATION, PRODUCTION) et les couches sélectionnées
--   (BRZ, SLV, GLD ou ALL).
--
--   Pour chaque combinaison (ENV × COUCHE) active, la procédure crée :
--     - La base de données BD_<COUCHE>_<ENV> (si absente)
--     - Le schéma cible
--     - Les 5 rôles d'accès RA (RO, RW, OW, EX, OPS) + hiérarchie + grants
--     - Les warehouses de la couche (BRZ uniquement : PBI, DBT, INGESTION,
--       DEV, ADMIN, ANALYST)
--     - Les rôles fonctionnels RF par défaut (si P_CREATE_DEFAULT_RF = TRUE)
--
-- NOMENCLATURE SCHÉMAS :
--   BRZ : SH_<STREAM>_OE_<APP_SOURCE>_<ENV_SUFFIX>  (APP_SOURCE requis)
--   SLV : SH_<STREAM>_OM_<ENV_SUFFIX>
--   GLD : SH_<STREAM>_<OFFRE>_<USE_CASE>_<ENV_SUFFIX>  (OFFRE/USE_CASE optionnels)
--
-- ENV_SUFFIX = C<ENV_PROJET> | H<ENV_PROJET> | P<ENV_PROJET>
--
-- NOMENCLATURE RÔLES RA :
--   BRZ : RA_BRZ_<STREAM>_<APP_SOURCE>_<DROIT>_<ENV_SUFFIX>
--   SLV : RA_SLV_<STREAM>_<DROIT>_<ENV_SUFFIX>
--   GLD : RA_GLD_<STREAM>[_<OFFRE>[_<USE_CASE>]]_<DROIT>_<ENV_SUFFIX>
--
-- NOMENCLATURE RÔLES RF (par défaut) :
--   BRZ : RF_BRZ_<STREAM>_<APP_SOURCE>_<PROFIL>_<ENV_SUFFIX>
--         PROFIL ∈ {STR, BAT, ANALYST, DEVLOPPEUR, EX, ADMIN, DBT}
--   SLV : RF_SLV_<STREAM>_<PROFIL>_<ENV_SUFFIX>
--         PROFIL ∈ {DEVLOPPEUR, ANALYST, EX, ADMIN, DBT, PBI}
--   GLD : RF_GLD_<STREAM>[_<OFFRE>[_<USE_CASE>]]_<PROFIL>_<ENV_SUFFIX>
--         PROFIL ∈ {DEVLOPPEUR, ANALYST, EX, ADMIN, PBI, DBT}
--
-- PARAMÈTRES :
--   P_STREAM            VARCHAR  - Code du stream (ex : ELEC, GAZ)
--   P_ENV_PROJET        VARCHAR  - Code projet sans préfixe C/H/P (ex : 001)
--   P_APP_SOURCE        VARCHAR  - Application source, requis pour BRZ (ex : MYAPP)
--   P_OFFRE             VARCHAR  - Offre commerciale, optionnelle pour GLD
--   P_USE_CASE          VARCHAR  - Cas d'usage, optionnel pour GLD
--   P_WH_TAILLE         VARCHAR  - Taille des warehouses : XS | S | M | L | XL
--                                  (défaut : XS)
--   P_SKIP_IF_EXISTS    BOOLEAN  - Réservé (défaut : TRUE)
--   P_CREATE_DEFAULT_RF BOOLEAN  - Créer les RF par défaut (défaut : TRUE)
--   P_COUCHE            VARCHAR  - Filtre couches : ALL | BRZ | SLV | GLD
--                                  ou combinaisons (ex : BRZ,SLV) (défaut : ALL)
--
-- RETOUR :
--   VARCHAR — résumé SUCCESS avec compteurs, ou message ERROR.
--
-- DÉPENDANCES :
--   BD_ADMIN_INFRA.SH_DEPLOY.LOG_DEPLOY
--
-- PRÉREQUIS :
--   Le rôle appelant doit disposer des privilèges SYSADMIN / SECURITYADMIN
--   (EXECUTE AS CALLER).
--
-- EXEMPLES D'APPEL :
--   -- Toutes couches, toutes envs
--   CALL BD_ADMIN_INFRA.SH_DEPLOY.SP_PROVISION_GRDF_V4('ELEC','001','MYAPP');
--
--   -- BRZ uniquement
--   CALL BD_ADMIN_INFRA.SH_DEPLOY.SP_PROVISION_GRDF_V4(
--       'ELEC','001','MYAPP',NULL,NULL,'XS',TRUE,TRUE,'BRZ');
--
--   -- GLD avec offre et use-case, warehouses M
--   CALL BD_ADMIN_INFRA.SH_DEPLOY.SP_PROVISION_GRDF_V4(
--       'ELEC','001',NULL,'OFFRE1','UC1','M',TRUE,TRUE,'GLD');
-- ============================================================================
    ...
    FOR e IN 0 TO ARRAY_SIZE(envs_list) - 1 DO
        ...
        IF (V_DO_BRZ) THEN
            ...
            FOR wh IN (SELECT * FROM (VALUES (V_WH_PBI), (V_WH_DBT), (V_WH_ING), (V_WH_DEV), (V_WH_ADMIN), (V_WH_ANALYST)) AS t(wh_name)) DO
                EXECUTE IMMEDIATE 'CREATE WAREHOUSE IF NOT EXISTS ' || wh.wh_name
                    || ' WAREHOUSE_SIZE = ''' || V_WH_SIZE_SF || ''' AUTO_SUSPEND = 60';
                V_CREATED := V_CREATED + 1;
            END FOR;
